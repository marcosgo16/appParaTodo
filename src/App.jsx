...
const cl = {
  ...
  navy: "#111827",
  camel: "#B68A4A",
  stone: "rgba(17,24,39,.55)",
  white: "#FFFFFF",
  border: "rgba(17,24,39,.10)",
  tag: "rgba(17,24,39,.06)",
  danger: "#D92D20",
};

const BASE_S = {
  page:     { minHeight:"100vh", background:cl.cream, color:cl.navy, fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,'Apple Color Emoji','Segoe UI Emoji'" },
  app:      { maxWidth:430, margin:"0 auto", minHeight:"100vh", display:"flex", flexDirection:"column" },
  hdrBar:   { background:cl.white, position:"sticky", top:0, zIndex:100, borderBottom:`1px solid ${cl.border}`, backdropFilter:"saturate(180%) blur(10px)" },
  hdr:      { padding:"18px 20px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 },
  hdrSync:  { fontSize:11, color:cl.stone, letterSpacing:".02em", whiteSpace:"nowrap" },
  hdrAuth:  { display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", justifyContent:"flex-end", maxWidth:200 },
  btnOut:   { fontSize:12, padding:"6px 10px", borderRadius:999, border:`1px solid ${cl.border}`, background:cl.white, color:cl.navy, cursor:"pointer", fontFamily:"inherit" },
  hdrSub:   { fontSize:12, letterSpacing:".01em", color:cl.stone, marginBottom:2 },
  hdrH1:    { fontSize:22, color:cl.navy, fontWeight:800, letterSpacing:"-.02em" },
  tabsBar:  { background:cl.cream, borderBottom:`1px solid ${cl.border}`, position:"sticky", top:0, zIndex:99 },
  tabs:     { display:"flex" },
  tab:      { flex:1, padding:"12px 4px", fontSize:12, fontWeight:700, letterSpacing:".02em", textAlign:"center", cursor:"pointer", border:"none", borderBottom:"2px solid transparent", background:"none", color:cl.stone },
  tabOn:    { color:cl.navy, borderBottom:`2px solid ${cl.navy}` },
  sec:      { padding:16, flex:1 },
  grid2:    { display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 },
  slot:     { background:cl.white, border:`1px solid ${cl.border}`, borderRadius:16, minHeight:110, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", cursor:"pointer", position:"relative", padding:12, textAlign:"center", boxShadow:"0 10px 30px rgba(0,0,0,.05)" },
  slotFill: { borderColor:"rgba(17,24,39,.20)" },
  slotLbl:  { fontSize:11, letterSpacing:".02em", color:cl.stone, marginBottom:5, fontWeight:700 },
  slotEm:   { fontSize:27, marginBottom:3 },
  slotName: { fontSize:12, fontWeight:700, lineHeight:1.3 },
  slotBrand:{ fontSize:10, color:cl.stone, marginTop:2, display:"flex", alignItems:"center", gap:3 },
  slotX:    { position:"absolute", top:8, right:8, width:22, height:22, borderRadius:"50%", background:"rgba(217,45,32,.12)", color:cl.danger, fontSize:12, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" },
  ...
  notesBox: { background:cl.white, border:`1px solid ${cl.border}`, borderRadius:16, padding:"12px 14px", marginBottom:12, boxShadow:"0 10px 30px rgba(0,0,0,.05)" },
  ...
  btn:      { flex:1, padding:12, borderRadius:14, fontFamily:"inherit", fontSize:14, fontWeight:800, cursor:"pointer", border:"none" },
  btnP:     { background:cl.navy, color:"#fff", boxShadow:"0 14px 30px rgba(17,24,39,.18)" },
  btnS:     { background:cl.white, color:cl.navy, border:`1px solid ${cl.border}` },
  ...
  secTitle: { fontSize:16, fontWeight:800, marginBottom:12, letterSpacing:"-.01em" },
  card:     { background:cl.white, border:`1px solid ${cl.border}`, borderRadius:18, padding:14, marginBottom:10, boxShadow:"0 10px 30px rgba(0,0,0,.05)" },
  cardHdr:  { display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:9 },
  cardName: { fontWeight:800, fontSize:14 },
  cardDate: { fontSize:10, color:cl.stone },
  chips:    { display:"flex", gap:6, flexWrap:"wrap" },
  chip:     { background:cl.tag, borderRadius:999, padding:"5px 10px", fontSize:11, display:"flex", alignItems:"center", gap:6 },
  ...
  fchip:    { whiteSpace:"nowrap", padding:"7px 12px", borderRadius:999, fontSize:12, fontWeight:800, letterSpacing:".01em", cursor:"pointer", border:`1px solid ${cl.border}`, background:cl.white, color:cl.stone },
  fchipOn:  { background:cl.navy, color:"#fff", border:`1px solid ${cl.navy}` },
  itemGrid: { display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 },
  itemCard: { background:cl.white, border:`1px solid ${cl.border}`, borderRadius:18, padding:"13px 11px", position:"relative", boxShadow:"0 10px 30px rgba(0,0,0,.05)" },
  itemEm:   { fontSize:28, marginBottom:7, display:"block" },
  itemName: { fontSize:12, fontWeight:800, lineHeight:1.3, marginBottom:2 },
  ...
  itemDel:  { position:"absolute", top:10, right:10, width:26, height:26, borderRadius:"50%", background:"rgba(217,45,32,.12)", color:cl.danger, fontSize:12, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" },
  formBox:  { background:cl.white, border:`1px solid ${cl.border}`, borderRadius:20, padding:17, marginBottom:15, boxShadow:"0 10px 30px rgba(0,0,0,.05)" },
  formH3:   { fontSize:16, fontWeight:900, marginBottom:15, letterSpacing:"-.01em" },
  fg:       { marginBottom:12 },
  flbl:     { display:"block", fontSize:12, fontWeight:800, letterSpacing:".01em", color:cl.stone, marginBottom:6 },
  finput:   { width:"100%", padding:"11px 12px", border:`1px solid ${cl.border}`, borderRadius:14, fontFamily:"inherit", fontSize:14, color:cl.navy, background:cl.white, outline:"none", WebkitAppearance:"none" },
  fsel:     { width:"100%", padding:"11px 12px", border:`1px solid ${cl.border}`, borderRadius:14, fontFamily:"inherit", fontSize:14, color:cl.navy, background:cl.white, outline:"none", WebkitAppearance:"none" },
  ep:       { display:"flex", flexWrap:"wrap", gap:7, marginTop:3 },
  eo:       { width:40, height:40, borderRadius:14, border:`1px solid ${cl.border}`, background:cl.white, fontSize:19, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" },
  eoOn:     { border:`1px solid rgba(17,24,39,.35)`, background:cl.white },
  ...
  overlay:  { position:"fixed", inset:0, background:"rgba(0,0,0,.35)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center" },
  modal:    { background:cl.white, borderRadius:"22px 22px 0 0", padding:20, width:"100%", maxWidth:430, maxHeight:"72vh", overflowY:"auto", boxShadow:"0 -20px 60px rgba(0,0,0,.18)" },
  mHandle:  { width:34, height:4, background:cl.border, borderRadius:2, margin:"0 auto 15px" },
  mTitle:   { fontSize:17, fontWeight:900, marginBottom:3, letterSpacing:"-.01em" },
  ...
  mItem:    { background:cl.cream, border:`1px solid ${cl.border}`, borderRadius:16, padding:"11px 9px", cursor:"pointer", textAlign:"center" },
  ...
  toast:    { position:"fixed", bottom:22, left:"50%", transform:"translateX(-50%) translateY(70px)", background:"rgba(17,24,39,.92)", color:"#fff", padding:"10px 16px", borderRadius:999, fontSize:13, fontWeight:800, zIndex:999, whiteSpace:"nowrap", pointerEvents:"none", transition:"transform .3s" },
  toastShow:{ transform:"translateX(-50%) translateY(0)" },
  fab:      { position:"fixed", right:18, bottom:18, zIndex:150, border:"none", background:cl.navy, color:"#fff", width:56, height:56, borderRadius:18, cursor:"pointer", boxShadow:"0 18px 44px rgba(17,24,39,.22)", display:"flex", alignItems:"center", justifyContent:"center" },
    ...
    <div style={S.page}>
      ...
      <div style={S.hdrBar}>
        <div ref={hdrMeasureRef} style={{ ...S.hdr, maxWidth: S.app.maxWidth, margin: "0 auto" }}>
          <div><div style={S.hdrSub}>Outfit</div><div style={S.hdrH1}>Outfit Maker</div></div>
  ...
  {aiModal && (
    <div style={S.overlay} onClick={e => { if (e.target === e.currentTarget) setAiModal(null); }}>
      <div style={{...S.modal, maxHeight:"80vh", display:"flex", flexDirection:"column"}}>
        <div style={S.mHandle} />
        <div style={{...S.mTitle, marginBottom:4}}>Asistente de estilo</div>
